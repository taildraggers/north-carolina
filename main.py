"""Aggregates North Carolina-located listings from every taildraggers.com
...
ones whose location ends in ", NC".
...
Jersey/New Mexico/New York repos' approach, with the state filter swapped
to "NC".
"""
...
PAGE_TITLE = "Taildragger Ads in North Carolina"
STATE_ABBR = "NC"
...
_NORTH_CAROLINA_LOCATION_RE = re.compile(r",\s*NC\s*$", re.IGNORECASE)


def _is_north_carolina(location: str) -> bool:
    return bool(_NORTH_CAROLINA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        north_carolina_rows = [row for row in rows if _is_north_carolina(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(north_carolina_rows)} in North Carolina")
        all_listings.extend(north_carolina_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No North Carolina listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique North Carolina listings: {len(listings)}")
