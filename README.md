...
Daily aggregator of North Carolina-located taildragger classified
...
[New York](https://github.com/taildraggers/new-york) repos' approach,
with the state filter swapped to "NC".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, NC` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in North Carolina"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/north-carolina/`
...
```html
<iframe
  src="https://taildraggers.github.io/north-carolina/"
  title="Taildragger Ads in North Carolina"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_NORTH_CAROLINA_LOCATION_RE` in `main.py`, and
  updating `PAGE_TITLE`.
